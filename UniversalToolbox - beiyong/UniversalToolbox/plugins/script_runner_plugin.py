...
import io
import contextlib
from tkinter import scrolledtext # 确保导入了scrolledtext
import traceback # 用于捕获堆栈信息
...
PLUGIN_META = {
    ...
    "description": "Browse and execute Python scripts from the 'scripts/' directory with improved output capture.",
    "author": "Gemini AI"
}
...
class ScriptRunnerPlugin:
    ...
    def __init__(self, app, parent_frame):
        ...
        # 假设脚本目录在项目根目录下的 'scripts' 文件夹
        self.script_dir = config.APP_DIR / "scripts" 
    ...
    def _build_ui(self):
        ...
        # --- 优化 1: 更改 Treeview 以显示列标题和数据 ---
        self.script_tree = ttk.Treeview(list_frame, 
                                        show="headings", 
                                        columns=('Name', 'Size'), 
                                        selectmode="browse")
        
        # 定义列
        self.script_tree.heading('Name', text='Script Name', anchor=tk.W)
        self.script_tree.column('Name', width=200, stretch=tk.YES, anchor=tk.W)
        self.script_tree.heading('Size', text='Size', anchor=tk.E)
        self.script_tree.column('Size', width=60, stretch=tk.NO, anchor=tk.E)
    ...
    def _refresh_script_list(self):
        ...
        for p in sorted(self.script_dir.glob("*.py")):
            ...
            # 使用 iid 存储完整路径，并使用 values 填充列数据
            self.script_tree.insert("", "end", 
                                     iid=str(p.resolve()), 
                                     values=(p.name, size_str)) 
    ...
    def _on_script_select(self, event):
        ...
        path_str = selected_id # Treeview 的 iid 存储了完整路径
        ...
        if name:
            ...
            try:
                ...
                initial_content = f'# Script: {name}\n\n' \
                                 f'# Note: You can access the main application instance via the global variable `app`\n' \
                                 f'# You can access the config module via the global variable `config`\n\n' \
                                 f'import sys\n' \
                                 f'import config as config_mod # Import as an alias to prevent conflict with global variable\n\n' \
                                 f'print("Hello from {name}!")\n' \
                                 f'config_mod.log("Example log from script", tag="script")\n'
    ...
    def _execute_script_thread(self, script_path: pathlib.Path):
        ...
        # 暂时修改 sys.path，确保脚本可以导入项目根目录下的模块
        ...
        try:
            # --- 优化 3: 使用 contextlib 简化输出重定向 ---
            with contextlib.redirect_stdout(capture_output), contextlib.redirect_stderr(capture_output):
            ...
            # --- 优化 4: 显式捕获 SystemExit ---
            self.app.log_to_console(f"Script {script_path.name} called sys.exit().", tag='warning')
        except Exception as e:
            # 捕获运行时错误
            self.app.log_to_console(f"Runtime Error in {script_path.name}: {type(e).__name__}: {e}", tag='error')
            # 捕获堆栈信息
            error_trace = traceback.format_exc()
            # 将完整的 traceback 写入捕获输出，以便在 finally 块中一起发送
            capture_output.write(f"\n--- Exception Traceback ---\n{error_trace}\n--- End Traceback ---\n")
        finally:
            # 恢复 sys.path
    ...
    def _execution_finished(self):
        ...
        self.is_running = False
        # 只有在当前脚本仍然是选定脚本时才恢复按钮状态
        if self.current_script_path and self.script_tree.focus() == str(self.current_script_path):
             self.run_btn.config(state=tk.NORMAL, text="▶ Run Selected Script")
        else:
             self.run_btn.config(state=tk.DISABLED, text="▶ Run Selected Script")
